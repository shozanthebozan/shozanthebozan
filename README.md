...
*  I’m currently working on `8vert` (a CLI unit-converting python program) , `FunkyVLC` (A C program that plays your music with cvlc in the terminal-with a clean look and interface) ,`Searchy` (a program for searching for files on Unix-like operating systems, the CLI version uses `bash` and the GUI version uses `python` with `tkinter` and `bash`) and `onNow` (a python GUI and CLI program to fetch latest weather for your location, and global news) .
